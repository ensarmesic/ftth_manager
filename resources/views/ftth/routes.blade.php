...
@section('subtitle', 'Dužine mikrocijevi, optičkog kabla i status izgradnje.')
...
<section class="mb-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
    ...
    <article class="rounded-md border border-zinc-200 bg-white p-4 shadow-sm"><div class="text-sm text-zinc-500">Optički kabl</div><div class="mt-2 text-2xl font-semibold">{{ number_format($routeStats['fiber']) }} m</div><div class="text-xs text-zinc-500">sa 10%: {{ number_format($routeStats['fiber_with_reserve']) }} m</div></article>
    <article class="rounded-md border border-zinc-200 bg-white p-4 shadow-sm"><div class="text-sm text-zinc-500">Ormarići / splitteri</div><div class="mt-2 text-2xl font-semibold">{{ $routeStats['planned_cabinets'] }} / {{ $routeStats['planned_splitters'] }}</div></article>
</section>
...
<section class="grid gap-6 xl:grid-cols-[420px_1fr]">
    <form method="POST" action="{{ route('routes.store') }}" class="rounded-md border border-zinc-200 bg-white p-5 shadow-sm">
        ...
        <div class="grid gap-4">
            ...
            <label class="grid gap-1 text-sm"><span>Početni ODF</span><select name="odf_id" class="rounded-md border border-zinc-300 px-3 py-2"><option value="">Bez ODF veze</option>@foreach($odfs as $odf)<option value="{{ $odf->id }}">{{ $odf->name }}</option>@endforeach</select></label>
            <label class="grid gap-1 text-sm"><span>Krajnji ormarić</span><select name="cabinet_id" class="rounded-md border border-zinc-300 px-3 py-2"><option value="">Bez ormarića</option>@foreach($cabinets as $cabinet)<option value="{{ $cabinet->id }}">{{ $cabinet->name }}</option>@endforeach</select></label>
            ...
            <label class="grid gap-1 text-sm"><span>Polaganje</span><select name="installation_type" class="rounded-md border border-zinc-300 px-3 py-2"><option value="underground">Podzemna</option><option value="aerial">Zračna</option></select></label>
            ...
            <button class="rounded-md bg-emerald-600 px-4 py-2 font-semibold text-white hover:bg-emerald-700">Sačuvaj trasu</button>
        </div>
    </form>

    @include('ftth.partials.table', ['rows' => $routes, 'columns' => ['name' => 'Trasa', 'project.name' => 'Projekat', 'odf.name' => 'ODF', 'cabinet.name' => 'ODO', 'installation_type' => 'Polaganje', 'microduct_type' => 'Mikrocijev', 'duct_length_m' => 'Dužina m', 'fiber_count' => 'Niti', 'status' => 'Status'], 'editRoute' => fn($id) => route('routes.update', $id), 'deleteRoute' => fn($id) => route('routes.delete', $id), 'editFields' => [
        'name' => 'Trasa',
        'route_type' => ['label' => 'Tip', 'type' => 'select', 'options' => ['feeder' => 'Feeder', 'distribution' => 'Distribucija', 'drop' => 'Drop']],
        'odf_id' => ['label' => 'ODF', 'type' => 'select', 'options' => ['' => 'Bez ODF veze'] + $odfs->pluck('name', 'id')->all()],
