...
<section class="mb-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
    <article class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <div class="mt-2 text-2xl font-semibold">{{ $materialStats['total'] }}</div>
    </article>
    <article class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        <div class="text-sm text-slate-500">Planirano KM</div>
        <div class="mt-2 text-2xl font-semibold">{{ number_format($materialStats['planned_cost'], 2) }}</div>
    </article>
    <article class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        <div class="text-sm text-slate-500">Utrošeno KM</div>
        <div class="mt-2 text-2xl font-semibold">{{ number_format($materialStats['used_cost'], 2) }}</div>
    </article>
    <article class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        <div class="text-sm text-slate-500">Projekti</div>
        <div class="mt-2 text-2xl font-semibold">{{ $materialStats['projects'] }}</div>
    </article>
</section>
