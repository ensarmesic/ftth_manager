...
try {
    ...
    process.stderr.write(`✗ Aplikacija nije dostupna na ${baseUrl} (${error.message}).\n`);
    process.exit(1);
}
...
try {
    ...
    for (const [label, path] of pages) {
        ...
        const onConsole = (message) => {
            ...
            const isBlockedExternalMapResource = text.includes('Failed to load resource: net::ERR_NETWORK_ACCESS_DENIED');
            if (message.type() === 'error' && !isBlockedExternalMapResource) consoleErrors.push(text);
        };
        ...
        try {
            ...
            if (!response || response.status() >= 400) {
                throw new Error(`Glavni dokument vraća HTTP ${response?.status() ?? 'bez odgovora'}`);
            }
            if (page.url().includes('/prijava')) throw new Error('Neočekivano preusmjerenje na prijavu');
            ...
            if (bodyText.length < 20) throw new Error('Stranica nema očekivani vidljivi sadržaj');
            ...
            await page.screenshot({ path: `storage/logs/pages-smoke-${slug}.png`, fullPage: true });
    ...
    process.stdout.write(`✓ Svih ${pages.length} glavnih stranica prošlo je browser smoke test.\n`);
} catch (error) {
    process.stderr.write(`✗ Browser smoke test nije prošao: ${error.message}\n`);
    process.exitCode = 1;
} finally {
    await browser.close();
}
