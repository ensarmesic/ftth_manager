...
@php
    $allChecks = $projects->map(function($project) {
        ...
        $project->routes->filter(fn ($r) => $r->route_type !== 'trench' && !$r->hasCompleteMicroductData())->each(fn ($r) => $checks->push(['warning', "{$r->name} nema kompletne podatke o mikrocijevi (tip, broj ili dužina).", route('map.dashboard', ['project' => $project->id, 'focus_type' => 'route', 'focus_id' => $r->id]), 'Prikaži na mapi']));
        $project->routes->filter(fn ($r) => $r->route_type !== 'trench' && $r->hasIncompleteCableData())->each(fn ($r) => $checks->push(['warning', "{$r->name} ima broj vlakana, ali nema dužinu kabla.", route('map.dashboard', ['project' => $project->id, 'focus_type' => 'route', 'focus_id' => $r->id]), 'Prikaži na mapi']));
