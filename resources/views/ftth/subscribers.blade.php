...
<section class="grid gap-6 xl:grid-cols-[420px_1fr]">
    <form method="POST" action="{{ route('subscribers.store') }}" class="rounded-md border border-zinc-200 bg-white p-5 shadow-sm">
        ...
        <div class="grid gap-4">
            ...
            <button type="submit" id="submit-btn" class="rounded-md bg-emerald-600 px-4 py-2 font-semibold text-white hover:bg-emerald-700">Sacuvaj korisnika</button>
        </div>
    </form>

    @include('ftth.partials.table', ['rows' => $subscribers, 'columns' => ['name' => 'Korisnik', 'project.name' => 'Projekat', 'cabinet.name' => 'Ormaric', 'address' => 'Adresa', 'service_status' => 'Status'], 'deleteRoute' => fn($id) => route('subscribers.delete', $id)])
</section>
