...
<div class="dashboard-root grid min-h-0 flex-1 gap-2 lg:grid-cols-[minmax(0,1fr)_250px] xl:grid-cols-[minmax(0,1fr)_270px]">
 ...
 <aside class="detail-column grid min-h-0 gap-2 overflow-auto md:grid-cols-2 lg:grid-cols-1 lg:overflow-auto">
  <section id="detail-panel" class="panel overflow-hidden"><div id="detail-heading" class="flex items-center justify-between p-3"><div class="flex items-center gap-3"><span class="rounded bg-green-100 p-2 text-sm font-bold text-green-700">ODO</span><b>{{ $selectedCabinet->name ?? 'Nema ODO ormarica' }}</b></div>@if($selectedCabinet)<span class="rounded bg-green-100 px-2 py-1 text-[10px] font-bold text-green-700">Aktivan</span>@endif</div>
   ...
   <section class="panel project-stats overflow-hidden"><h2>STATISTIKA PROJEKTA</h2><div class="project-stats-list">@foreach([['routes_m','Ukupna duzina trasa',$stats['routes_m']],['microduct_14_10','Mikrocijev 14/10',$stats['microduct_14_10']],['microduct_10_8','Mikrocijev 10/8',$stats['microduct_10_8']],['fiber_4','Opticki kabal 4 niti',$stats['fiber_4']],['fiber_12','Opticki kabal 12 niti',$stats['fiber_12']]] as $item)<div><span>{{ $item[1] }}</span><b data-stat="{{ $item[0] }}" data-value="{{ $item[2] }}">{{ number_format($item[2]/1000, 2) }} km</b></div>@endforeach @foreach([['splitters','Splitteri (1:4)',$stats['splitters']],['cabinets','ODO ormarici',$cabinets->count()],['houses','Kuce',$houses->count()],['subscribers','Korisnici',$stats['subscribers']]] as $item)<div><span>{{ $item[1] }}</span><b data-count-stat="{{ $item[0] }}">{{ $item[2] }}</b></div>@endforeach</div><a href="{{ route('reports.index') }}">Detaljan izvjestaj</a></section>
 </aside>
</div>
