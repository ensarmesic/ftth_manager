...
<section class="mb-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
    <article class="stat-card">
        <div class="flex items-start justify-between gap-3">
            <div>
                <div class="stat-label">Kuće ukupno</div>
                <div class="stat-value">{{ $houseStats['total'] }}</div>
            </div>
    ...
    <article class="stat-card">
        <div class="flex items-start justify-between gap-3">
            <div>
                <div class="stat-label">Povezane na ODO</div>
                <div class="stat-value">{{ $houseStats['connected'] }}</div>
            </div>
    ...
    <article class="stat-card">
        <div class="flex items-start justify-between gap-3">
            <div>
                <div class="stat-label">Bez ormarića</div>
                <div class="stat-value">{{ $houseStats['unassigned'] }}</div>
            </div>
    ...
    <article class="stat-card">
        <div class="flex items-start justify-between gap-3">
            <div>
                <div class="stat-label">Raspoloživi ODO</div>
                <div class="stat-value">{{ $houseStats['cabinets'] }}</div>
            </div>
...
<section class="grid gap-6 xl:grid-cols-[420px_1fr]">
    <form method="POST" action="{{ route('houses.store') }}" class="page-form">
        @csrf
            ...
            <label class="ftth-label">ODO ormarić
                <select name="cabinet_id" class="ftth-input">
                    ...
                    @foreach($cabinets as $cabinet)
                        <option value="{{ $cabinet->id }}" @disabled($cabinet->houses_count >= 12)>{{ $cabinet->name }} — {{ $cabinet->project->name }} ({{ $cabinet->houses_count }}/12)</option>
                    @endforeach
                </select>
            </label>
