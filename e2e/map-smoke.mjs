...
try {
    ...
    process.stderr.write("  Pokreni 'composer run dev' u drugom terminalu, pa ponovo.\n");
    process.exit(1);
}
...
try {
    ...
    const hasMap = await page.evaluate(() => typeof window.ftthNetworkMap === 'object' && window.ftthNetworkMap !== null);
    if (!hasMap) fail('window.ftthNetworkMap nije inicijalizovan (init.js nije prošao).');
    ...
    if (command.length === 0) fail('Toolbar nije spreman (#cad-command je prazan).');

    // 4. No uncaught JS error during boot.
    if (pageErrors.length > 0) fail(`Uncaught JS greške: ${pageErrors.join(' | ')}`);

    if (!process.exitCode) {
        process.stdout.write(`✓ Mapa se učitava i inicijalizuje bez grešaka (${target}).\n`);
    }
} catch (error) {
